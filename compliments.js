...
Module.register("compliments", {
	...
	defaults: {
		compliments: {
            anytime: ["Hey there, gorgeous!"],
            morning: ["Good morning, superstar!", "Rise and shine!"],
            afternoon: ["You're doing amazing!", "Keep shining!"],
            evening: ["You light up the room!", "Looking sharp!"],
            "2024-01-01": ["Happy New Year! Wishing you joy!"],
		},
        users: {
            "Sona": ["You're amazing, Sona!", "Keep up the great work, Sona!"],
            "Akshay": ["You're a star, Akshay!", "Fantastic job today, Akshay!"]
	 ...
	 */
    getRandomCompliment() {
        // Check if there's a recognized user
        const userCompliments = this.config.compliments.users[this.currentUser] || [];
        const compliments = this.complimentArray();
    
        let index;
        if (userCompliments.length > 0) {
            // Get a compliment specific to the recognized user
            index = this.randomIndex(userCompliments);
            return userCompliments[index] || "";
    ...
    notificationReceived(notification, payload, sender) {
        ...
        } else if (notification === "FACE_RECOGNIZED") {
            this.currentUser = payload.name; // Capture recognized user name
            this.updateDom(); // Update the displayed compliment
        }
    },
});
